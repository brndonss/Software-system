...
const navItems = ["How it works", "Platform", "Automations", "AI", "Security"];
...
export default function Home() {
  const [showAuth, setShowAuth] = useState(false); const [authMode, setAuthMode] = useState<"sign-in" | "create">("create"); const [menuOpen, setMenuOpen] = useState(false); const [activeWorkflow, setActiveWorkflow] = useState(0);
  function openAuth(mode: "sign-in" | "create") { setAuthMode(mode); setShowAuth(true); setMenuOpen(false); }
  function goTo(id: string) { document.getElementById(id)?.scrollIntoView({ behavior: "smooth" }); setMenuOpen(false); }
  return <main className="ns-landing">
    <header className={`ns-header ${menuOpen ? "is-open" : ""}`}><button className="ns-logo" onClick={() => goTo("top")} aria-label="Northstar home"><span>N</span><b>NORTHSTAR</b></button><nav className="ns-nav" aria-label="Primary navigation">{navItems.map((item) => <button key={item} onClick={() => goTo(item === "How it works" ? "how-it-works" : item.toLowerCase())}>{item}</button>)}</nav><div className="ns-header-actions"><button className="ns-signin" onClick={() => openAuth("sign-in")}>Sign in</button><button className="ns-button ns-button-dark" onClick={() => openAuth("create")}>Build My System <span>↗</span></button></div><button className="ns-menu" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation"><i /><i /></button></header>
