-- Allow failed provisioning operations to return to pending only inside
-- retry_provisioning_job. Ordinary updates remain rejected by the trigger.
...
begin
  if old.state <> new.state and not (
    ...
    or (
      ...
      and current_setting('northstar.retry_provisioning_job_id', true) = old.job_id::text
    )
  ) then
...
begin
  ...
  perform set_config('northstar.retry_provisioning_job_id', job_row.id::text, true);
