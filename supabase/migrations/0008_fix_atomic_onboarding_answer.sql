...
begin
  ...
  into session_status
  from public.onboarding_sessions as session_row
  ...
  select answer_row.id, answer_row.version
  into previous_id, answer_version
  from public.onboarding_answers as answer_row
  where answer_row.onboarding_session_id = p_session_id
    and answer_row.customer_id = p_customer_id
    and answer_row.question_key = p_question_key
    and answer_row.is_latest = true
  for update;
  ...
  if found then
    ...
    set is_latest = false,
        ...
        updated_at = now()
    where onboarding_answers.id = previous_id;
  else
    answer_version := 0;
  end if;
